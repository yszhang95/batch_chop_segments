...
def chop_torch(X0X1, Ns, result):
    ...
    for i in range(len(Ns)):
        ...
        result[idx:idx+size,0] = xs[:-1]
        result[idx:idx+size,1] = ys[:-1]
        result[idx:idx+size,2] = zs[:-1]
        result[idx:idx+size,3] = xs[1:]
        result[idx:idx+size,4] = ys[1:]
        result[idx:idx+size,5] = zs[1:]
